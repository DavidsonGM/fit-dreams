...
class LessonsController < ApplicationController
  ...
  def create
    ...
    lesson.save!
    render json: { message: "Aluno #{lesson.user.name} matriculado na aula de #{lesson.gym_class.name}" },
           status: :created
  ...
  def delete
    ...
    lesson.destroy!
    render json: { message: "Aluno #{lesson.user.name} desmatriculado da aula de #{lesson.gym_class.name}" },
           status: :created
  rescue StandardError => e
    render json: { error: e }, status: :bad_request
  end
